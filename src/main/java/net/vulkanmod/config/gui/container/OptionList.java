...
public class OptionList extends AbstractScrollableList {
    ...
    private void renderWidgetTooltip(ControllerWidget<?> widget) {
        ...
        Dimension<Integer> tooltipDim = Dimension.ofInt(
                widget.dim.xLimit() - TOOLTIP_WIDTH, widget.dim.yLimit() + 3,
        ...
        if ((tooltipDim.yLimit() - getScrollAmount()) > dim.yLimit())
            tooltipDim = tooltipDim.withY(widget.dim.y() - tooltipDim.height() - 3);
        if ((tooltipDim.y() - getScrollAmount()) < 0)
            tooltipDim = tooltipDim.withY(widget.dim.yLimit());
    ...
    public void addChildren() {
        ...
        for (OptionGroup group : page.groups()) {
            ...
            for (Option<?> option : group.options()) {
                Dimension<Integer> widgetDim = Dimension.ofInt(x, topY, dim.width(), GuiConstants.WIDGET_HEIGHT);
