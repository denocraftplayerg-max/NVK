...
public class PageList extends AbstractWidgetContainer {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        GuiRenderer.fill(
                dim.x(), dim.y(),
                dim.xLimit(), dim.y() + GuiConstants.WIDGET_HEIGHT + GuiConstants.WIDGET_MARGIN,
                backgroundColor);

        GuiRenderer.guiGraphics.blit(
                ICON,
                dim.centerX() - ICON_SIZE / 2, dim.y() + (((GuiConstants.WIDGET_HEIGHT + GuiConstants.WIDGET_MARGIN) - ICON_SIZE) / 2),
                0, 0,
    ...
    protected void addChildren() {
        ...
        int topY = dim.y() + GuiConstants.WIDGET_HEIGHT + GuiConstants.WIDGET_MARGIN;
        for (OptionPage page : pages) {
            Dimension<Integer> buttonDim = Dimension.ofInt(
                    dim.x(), topY,
                    dim.width(), GuiConstants.WIDGET_HEIGHT);
