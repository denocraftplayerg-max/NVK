...
public abstract class AbstractScrollableList extends AbstractWidgetContainer {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        if (getMaxScrollAmount() > 0) {
            dim = dim.withWidth(viewPortDim.width() - 5);
            renderScrollBar();
        }
    ...
    public boolean isMouseOver(double mouseX, double mouseY) {
        return viewPortDim.isPointInside(mouseX, mouseY);
    }

    @Override
    public @NotNull ScreenRectangle getRectangle() {
        return viewPortDim.rectangle();
    }
}
