...
public class SearchFieldWidget extends AbstractWidget {
    ...
    private void renderCursor() {
        ...
        int cursorPos = dim.x() + 8 + GuiRenderer.font.width(visibleQuery.substring(0, model.getCursor()));
        GuiRenderer.drawString(
                Component.literal("_"),
                cursorPos, dim.centerY() - GuiRenderer.font.lineHeight / 2,
                GuiConstants.COLOR_WHITE);
    }
    ...
    private void renderSelection() {
        ...
        if (selectionEnd != selectionStart) {
            ...
            int x0 = dim.x() + 8 + GuiRenderer.font.width(visibleQuery.substring(0, selectionStart));
            int x1 = dim.x() + 8 + GuiRenderer.font.width(visibleQuery.substring(0, selectionEnd));
            int y0 = dim.centerY() - GuiRenderer.font.lineHeight / 2;
            int y1 = dim.centerY() + GuiRenderer.font.lineHeight / 2;

            GuiRenderer.fill(
                    Math.min(Math.min(x0, x1), dim.xLimit()), Math.min(y0, y1),
                    Math.min(Math.max(x0, x1), dim.xLimit()), Math.max(y0, y1),
                    selectionColor);
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int offsetX = Mth.floor(mouseX) - dim.x() - 8;
        String visibleString = GuiRenderer.font.plainSubstrByWidth(model.getQuery().substring(model.getFirstCharacterIndex()), dim.width() - 16);
